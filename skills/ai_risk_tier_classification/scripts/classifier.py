...
# NOTE: this is a keyword-based approximation of the classification the
# ADK Risk Classifier Agent would produce via a live LLM call querying the
# MCP framework server (agents/risk_classifier.py). It's the offline path
# actually exercised in this environment (no ANTHROPIC_API_KEY configured),
# and what powers this skill's CLI/standalone use -- but its citation
# strings are hand-picked here, not sourced live from
# mcp_server/frameworks/*.json the way a real MCP tool call would format
# them, so they can drift from the live agent's exact phrasing even when
# the underlying tier/category is the same. Reconciling the two citation
# formats is a Day 5 polish item, not addressed here.
